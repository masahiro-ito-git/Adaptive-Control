% 例3.3を相対次数2にしたもの
function dx_all=model_3_2MRAC(t,x_all,r)
% x_all=[y;ym;theta;v1;v2;xi]
...
v1_cell=cell(2,1); v2_cell=cell(2,1); xi_cell=cell(10,1); x_z_cell=cell(2,1);
[y_cell{:},ym_cell{:},theta_cell{:},v1_cell{:},v2_cell{:},...
    xi_cell{:},b0hat,x_z_cell{:}]=x_all_cell{:};
...
xi=cell2mat(xi_cell); xi=xi(:);
x_z=cell2mat(x_z_cell); x_z=x_z(:);

omega=[r;v1;v2];
u=theta' * omega;
m_sq=lambda0+xi(1:5)'*xi(1:5);
z=x_z(1)+theta'*xi(1:5);
e=y(1)-ym(1);
ehat=b0hat*z;
ea=e-ehat;
...
dv1=F*v1+g*u;
dv2=F*v2+g*y(1);
dxi=[zeros(5),eye(5);-eye(5),-2*eye(5)]*xi+[zeros(5,1);omega];
db0hat=g1*z/m_sq*ea;
...
if t>=inf
  ...
  dtheta=G2*xi(1:5)/m_sq*ea;
end

dx_all=[dy;dym;dtheta;dv1;dv2;dxi;db0hat;dx_z];
